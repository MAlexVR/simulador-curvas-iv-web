import { ModuleParams, SimulationResults, ChartDataPoint, ModelType } from "@/types/module";
...
function modeloDoubleDiode(
  ...
  for (const v of V) {
    ...
    for (let iter = 0; iter < 100; iter++) {
      ...
      const df = -(I_o * Rs / (A1 * Ns * Vt)) * exp1 - 
                 (I_o * Rs / (A2 * Ns * Vt)) * exp2 - Rs / Rsh - 1;
...
function modeloTripleDiode(
  ...
  for (const v of V) {
    ...
    for (let iter = 0; iter < 100; iter++) {
      ...
      const df = -(I_o * Rs / (Ns * Vt)) * (exp1/A1 + exp2/A2 + exp3/A3) - Rs / Rsh - 1;
...
export const MODEL_NAMES: Record<ModelType, string> = {
  ...
  'lambert': 'Lambert W - Barry Analytical'
};
...
export function runSimulation(params: ModuleParams): SimulationResults {
  ...
  const { I_ph, Io_op, Rs_op, Rsh_op } = calcularParametrosTermicos(
    Isc_ref, Voc_ref, Rs_ref, Rsh_ref, n, Ns, G_op, T_op_C, alpha, beta
  );
  ...
  switch (modelo) {
    case 'sdm':
      current = modeloSingleDiode(voltage, I_ph, Io_op, n, Ns, Rs_op, Rsh_op, T_op);
      modelName = MODEL_NAMES.sdm;
      break;
    case 'ddm':
      const A1 = 1.0, A2 = 2.0;
      current = modeloDoubleDiode(voltage, I_ph, Io_op, A1, A2, Rs_op, Rsh_op, Ns, T_op);
      modelName = MODEL_NAMES.ddm;
      break;
    case 'tdm':
      const A1_t = 1.0, A2_t = 1.2, A3_t = 2.5;
      current = modeloTripleDiode(voltage, I_ph, Io_op, A1_t, A2_t, A3_t, Rs_op, Rsh_op, Ns, T_op);
      modelName = MODEL_NAMES.tdm;
      break;
    case 'lambert':
    default:
      current = modeloLambertW(voltage, I_ph, Io_op, n, Ns, Rs_op, Rsh_op, T_op);
